  ...
  try {
    ...
    if (eventSlug) {
      var events = await sb.sbGet(
        ...
        '&select=id,name,event_date,format,handicap_allowance,sponsor_name,sponsor_logo_url,headline_text,status,board_rows_per_page,leaderboard_freeze_hole,board_show_full,counted_holes,results_published_at,auto_closed,charity_name,charity_line,charity_url,charity_logo_url' +
        '&limit=1'
      );
